...
# Common patterns for stock ticker (e.g. AAPL, 0AAPL, TSLA)
...
def parse_trade_text(text: str) -> Optional[Tuple[str, str, float, float]]:
    ...
    for pat in _DIRECTION_PATTERNS:
        if re.search(pat, text, re.IGNORECASE):
            direction = "BUY" if "BUY" in upper_pat(pat) or "買" in text else "SELL"
            break
...
def main() -> None:
    ...
    # Load config (connects to Google Sheets)
    manager = GoogleSheetsManager()
    data = manager.load_config()
    holdings: List[dict] = data["holdings"]

    # Build lookup: ticker → row
    ...
    if ticker not in holdings_map:
        print(f"\n⚠️  找不到 {ticker} 的持倉紀錄，請先在 Google Sheets 中新增。")
        sys.exit(1)

    old_row = holdings_map[ticker]
    old_shares = float(old_row.get("shares", 0))
    old_avg = float(old_row.get("avgcost", 0))
    ...
    confirm = input("\n確定要寫入 Google Sheets？[Y/n]: ").strip().lower()
    if confirm.startswith("n"):
        print("已取消。")
        sys.exit(0)

    manager.update_holdings(ticker, new_shares, new_avg)
    print("\n✅ 持倉已成功更新！")
